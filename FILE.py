...
def get_cookie(access_token):
    url = "https://graph.facebook.com/v13.0/me?access_token=" + access_token
    response = requests.get(url)
    if response.status_code == 200:
        json_data = response.json()
        user_id = json_data["id"]
        cookie = f"fbm_{user_id}=base_domain=.facebook.com; fbsr_{user_id}={access_token}"
        return cookie
    else:
...
if os.path.exists(filename):
    ...
    if cookie:
        ...
        jala("\033[1;32mchange your login ID");exit()
